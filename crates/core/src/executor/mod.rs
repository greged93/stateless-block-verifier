...
use revm::scroll::L1_GAS_PRICE_ORACLE_ADDRESS;
use revm::{
    db::CacheDB,
    primitives::{Env, B256, KECCAK_EMPTY, POSEIDON_EMPTY},
    State,
};
use sbv_primitives::{
...
pub struct EvmExecutor<'db, CodeDb, ZkDb> {
    ...
    db: State<CacheDB<EvmDatabase<'db, CodeDb, ZkDb>>>,
}
...
impl<CodeDb: KVDatabase, ZkDb: KVDatabase + 'static> EvmExecutor<'_, CodeDb, ZkDb> {
    ...
    fn handle_block_inner<T: Block>(
        ...
        let _ = self.db.load_cache_account(L1_GAS_PRICE_ORACLE_ADDRESS)?;
    ...
    fn commit_changes_inner(&mut self) -> Result<B256, DatabaseError> {
        let mut zktrie = ZkTrie::<Poseidon>::new_with_root(
            self.db.database.db.zktrie_db,
            NoCacheHasher,
            self.db.database.db.committed_zktrie_root(),
        )
        .expect("infallible");
        ...
        let mut debug_recorder = sbv_utils::DebugRecorder::new(
            ...
            self.db.database.committed_zktrie_root(),
        );

        for (addr, db_acc) in self.db.cache.accounts.iter_mut() {
            if let Some(db_acc) = &mut db_acc.account {
                let mut info = db_acc.info.clone();
                if info.is_empty() {
                    continue;
                }
                ...
                dev_trace!("committing {addr}, {db_acc:?}");
                cycle_tracker_start!("commit account");

                let mut storage_root = self.db.database.db.prev_storage_root(addr);
                ...
                if !db_acc.storage.is_empty() {
                    ...
                    measure_duration_micros!(
                        ...
                        cycle_track!(
                            storage_trie
                                .commit(self.db.database.db.zktrie_db)
                                .map_err(DatabaseError::zk_trie)?,
                            "Zktrie::commit"
                        )
                    );
                    ...
                    self.db
                        .database
                        .db
                        .update_storage_root_cache(*addr, storage_trie);
                ...
                measure_duration_micros!(
                    ...
                    cycle_track!(
                        zktrie
                            .update(self.db.database.db.zktrie_db, addr, acc_data)
        ...
        self.db
            .database
            .db
            .updated_committed_zktrie_root(root_after);

        self.db.merge_transitions(BundleRetention::Reverts);

        self.db.database.accounts.clear();
        self.db.database.contracts.clear();
        self.db.block_hashes.clear();
        self.db.database.logs.clear();
